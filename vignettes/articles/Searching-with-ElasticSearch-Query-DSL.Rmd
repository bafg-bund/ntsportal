...
`getTableByQuery()` implements the Python `elasticsearch` package in the back-end (via the `ntsportal::DbComm` interface). [Connecting to the NTSPortal Elasticsearch Search API from R or Python](articles/Connecting-to-NTSPortal-API.html) has more details regarding using the API in R.

See also `ntsportal::getTableByEsql()` for a simpler query language.
