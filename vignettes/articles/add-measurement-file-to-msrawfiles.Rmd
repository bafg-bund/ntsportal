...
`addRawfiles()` creates a new metadata entry for a measurment file in the `msrawfiles` table. It uses an existing entry as a template and dynamically changes certain fields based on data encoded in the filename or directly passed by the user. More details are found in the `addRawfiles()` documentation.

# Step-by-step guide to using `addRawfiles()`

1. Install the newest version of *ntsportal*, including the Python requirements.
2. First, create a backup of `msrawfiles`. Use the function `createBackupMsrawfiles()`, a backup named `ntsp25.3_msrawfiles_backup_<YYYYMMDD>` is then created.

```{r, eval=FALSE}

library(ntsportal)

rfIndex <- "ntsp25.3_msrawfiles"
...
5. Check `add-rawfiles-check.json` (e.g., `start`, `filename`, `pol`, `instrument`, `tag`, `comment` and `blank` fields)
...
## The files being added have different sampling locations

`addRawfiles()` can dynamically change The location for each entry based on the location encoded in the filename. The field `dbas_station_regex` in the template document is used to find documents from a matching location from existing entries in `msrawfiles`. The location information is then copied from these documents (`station`, `loc`, `river`, `km` and `gkz` fields). `dbas_station_regex` must have `()` to indicate where the station code is found in the filename, e.g., `^(.*)_\d{2}_\d{2}`. 

Note that if the sampling date (`start`) is also encoded in the filename, the parsing is done using the `start_date_regex` from the template entry. Therefore, the regular expression in `start_date_regex` from the template entry must be generalized to work with all possible station codes. 
