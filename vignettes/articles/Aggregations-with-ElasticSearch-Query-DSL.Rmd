...
The API documentation is extensive: [Aggregations Docs](https://www.elastic.co/docs/explore-analyze/query-filter/aggregations). For the NTSPortal schema, see [Structure of NTSPortal](articles/structure-and-table-naming.html) and [Document field descriptions for NTSPortal](articles/table-mappings.html).

The use of the aggregation API from Python is possible using the package `elasticsearch` (in R the python code is called via `reticulate`). See the example in `ntsportal::getUniqueValues()`.
...
This code (for the elasticsearch console) is to search for two stations and split results by station, polarity and blank yes/no via three nested `terms` aggregations. 
...
```json
GET ntsp25.2_dbas*/_search?size=0 
...
```
**Explanations:**  
`ntsp25.2_dbas*` -- with this index pattern, all indices starting with this character string are selected.  
...
```json
GET ntsp25.3_msrawfiles/_search?size=0
{
  "query": {
    "term": {
      "matrix": {
