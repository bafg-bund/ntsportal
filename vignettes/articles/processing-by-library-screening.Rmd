...
The user runs one of several `screening*` functions to start the processing either locally or via SLURM. In `screeningSelectedBatchesSlurm()` (the most commonly used) specific directories (i.e. batches) containing the measurement files are passed either by listing them directly or by giving a root directory. `dbas` is the default `screeningType` in these functions. For further details on selecting batches for processing, see Section [Collecting msrawfile records](#collecting-msrawfile-records).
...
![Figure: Details of `ingestFeatureRecords()`](images/screening-ingest.svg){width=100%}
