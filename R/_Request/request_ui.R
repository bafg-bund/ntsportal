request_ui <- function(id, df){ 
  ...
  fluidRow(
    ...
    column(3, wellPanel(
      ...
      selectInput(NS(id, "in_req_river"),  "(in progress) River input:", 
                  choices = NULL, multiple = TRUE),
      selectInput(NS(id, "in_req_matrix"), "(in progress) Matrix input:", 
                  choices = NULL, multiple = TRUE),
      selectInput(NS(id, "in_req_tag"),  "(in progress) Tag input:", 
                  choices = NULL, multiple = TRUE),
      selectInput(NS(id, "in_req_comp_group"),  "(in progress) Classification input:", 
                  choices = NULL, multiple = TRUE),
      selectInput(NS(id, "in_req_rtt_method"), "(in progress) Chrom. Meth.input:", 
                  choices = NULL, multiple = TRUE),
      selectInput(NS(id, "in_req_name"), "(in progress) Name input:", 
                  choices = NULL, multiple = TRUE),
