...
test_that("Batch check runs and returns error", {
  recordsInBatches <- readRDS(test_path("fixtures", "validation-record", "recordsInBatches.RDS"))
  expect_error(
...
test_that("Normal record validation returns true", {
  records <- readRDS(test_path("fixtures", "validation-record", "allRecordsFlat.RDS"))
